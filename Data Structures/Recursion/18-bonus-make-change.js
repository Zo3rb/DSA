...
// Helper function for makeBetterChange
function makeChangeHelper(target, coins) {
  if (target === 0) {
    return [];
  }
  if (coins.length === 0) {
  ...
  let bestChange = null;
  for (let numCoins = 0; numCoins * currentCoin <= target; numCoins++) {
    const thisChange = makeChangeHelper(
      target - numCoins * currentCoin,
      remainingCoins
    );
    if (thisChange !== null) {
      const change = Array(numCoins).fill(currentCoin);
      change.push(...thisChange);
...
function makeBetterChange(target, coins = [25, 10, 5, 1]) {
  const sortedCoins = coins.sort((a, b) => b - a); // Sort coins in descending order
  return makeChangeHelper(target, sortedCoins);
}
